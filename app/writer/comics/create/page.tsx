    ...
    const checkAndSeed = async () => {
      if (genres.length === 0) {
        try {
          ...
          const seedResponse = await fetch("/api/genres/seed", {
            method: "POST",
          });
  ...
  const fetchGenres = async () => {
    try {
      ...
      if (data.success && data.data?.genres) {
        ...
        console.log(
          "Main genres:",
          fetchedGenres.filter((g: Genre) => !g.slug.startsWith("sub-")).length,
        );
        console.log(
          "Sub genres:",
          fetchedGenres.filter((g: Genre) => g.slug.startsWith("sub-")).length,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        // Validate image dimensions (must not exceed 500x700 px)
        const dimOk = await new Promise<boolean>((resolve) => {
          ...
          img.onload = () => {
            ...
            if (width > 500 || height > 700) {
              ...
              setCoverError("ขนาดรูปต้องไม่เกิน 500x700 พิกเซล");
        ...
        if (nsfwModel) {
          try {
            ...
            // If NSFW content detected and isMature is false, block upload
            if (nsfwScore > 0.5 && !formData.isMature) {
              toast.error(
                "ตรวจพบเนื้อหา 18+ ในภาพ กรุณาเปลี่ยนภาพหรือตั้งค่าระดับเนื้อหาเป็น 18+",
              );
              setCoverPreview(null);
              setFormData((prev) => ({ ...prev, coverUrl: "" }));
              if (fileInputRef.current) fileInputRef.current.value = "";
              setCoverError(
                "ภาพนี้มีเนื้อหา 18+ กรุณาเปลี่ยนภาพหรือตั้งค่าระดับเนื้อหาเป็น 18+",
              );
              setCheckingNsfw(false);
              return;
            }

            // If NSFW content detected and isMature is true, show warning
            if (nsfwScore > 0.5 && formData.isMature) {
              toast.warning(
            ...
            // Set cover URL if passed all checks
            setFormData((prev) => ({ ...prev, coverUrl: dataUrl }));
          } catch (error) {
            console.error("Error checking NSFW content:", error);
            // If NSFW check fails, still allow upload but log error
            setFormData((prev) => ({ ...prev, coverUrl: dataUrl }));
            toast.warning(
          ...
          // If model not loaded yet, allow upload but show warning
          setFormData((prev) => ({ ...prev, coverUrl: dataUrl }));
          if (!checkingNsfw) {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        const errorMessage =
          data.error ||
          data.message ||
          `HTTP ${response.status}: เกิดข้อผิดพลาดในการสร้างมังงะ`;
      ...
      if (data.success && data.data?.manga) {
        ...
        const errorMessage =
          data.error || data.message || "เกิดข้อผิดพลาดในการสร้างมังงะ";
        toast.error(errorMessage);
        console.error("Create manga error:", data);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (genres.length > 0) {
      ...
      console.log(
        "📚 Main genres:",
        mainGenres.length,
        mainGenres.map((g) => g.name),
      );
      console.log(
        "📚 Sub genres:",
        subGenres.length,
        subGenres.map((g) => g.name),
      );
    } else {
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                <div className="space-y-3 text-sm text-foreground">
                  <ul className="space-y-2.5 list-disc list-inside">
                    <li>
                      เนื้อหาที่อัปโหลดต้องเป็นผลงานต้นฉบับของผู้สร้าง
                      ไม่ใช่การคัดลอกหรือลอกเลียนแบบผลงานของผู้อื่น
                    </li>
                    <li>
                      เนื้อหาที่อัปโหลดต้องไม่เป็นการหมิ่นประมาท ใส่ร้าย
                      หรือทำให้เกิดความเสียหายต่อบุคคลที่สาม
                    </li>
                    <li>
                      เนื้อหาที่อัปโหลดต้องไม่ละเมิดลิขสิทธิ์ของผู้อื่น
                      หากมีการอ้างอิงหรือใช้เนื้อหาของผู้อื่น
                      ต้องได้รับอนุญาตและมีหลักฐานการอนุญาตที่ชัดเจน
                    </li>
                    <li>
                      หากมีการละเมิดลิขสิทธิ์ RTN
                      ขอสงวนสิทธิ์ในการระงับหรือลบเนื้อหาดังกล่าว
                    </li>
                    <li>
                      เนื้อหาที่เคยเผยแพร่บนแพลตฟอร์มอื่นหรือในรูปแบบสิ่งพิมพ์สามารถอัปโหลดได้
                      หากผู้อัปโหลดมีสิทธิ์ในการเผยแพร่เนื้อหาดังกล่าว
                    </li>
                    <li>
                      ...
                      <ul className="ml-6 mt-1 space-y-1 list-disc">
                        ...
                        <li>
                          ภาพที่แสดงกิจกรรมทางเพศ อวัยวะเพศ หัวนม
                          หรือส่วนที่เห็นได้ชัดเจนของอวัยวะเพศผ่านเสื้อผ้า
                        </li>
                        <li>
                          ภาพที่แสดงความรุนแรงหรือภาพที่น่าตกใจ เช่น การทรมาน
                          เลือด หรือศพ
                        </li>
                        <li>
                          ภาพที่ถูกแก้ไขเพื่อหมิ่นประมาทหรือทำร้ายบุคคลที่สาม
                        </li>
                      </ul>
                    </li>
                    <li>
                      RTN ขอสงวนสิทธิ์ในการปรับแต่งหรือระงับผลงานของผู้สร้าง
                      หากมีการละเมิดข้อตกลงและเงื่อนไข
                    </li>
                    <li>
                      RTN
                      จะไม่รับผิดชอบต่อการฟ้องร้องหรือความเสียหายที่เกิดจากการละเมิดข้อตกลงและเงื่อนไขของผู้สร้าง
                    </li>
                    <li>
                      <Link
                        href="/terms"
              ...
              <div className="px-6 py-4 border-t border-border space-y-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-foreground">
                    ฉันยอมรับข้อตกลงและเงื่อนไขแล้ว
                  </span>
                </label>
              ...
              <div className="bg-card rounded-xl border border-border p-6 sticky top-6">
                ...
                <div
                  ...
                  className={cn(
                    ...
                    coverError && "!border-red-500 bg-red-500/10",
                  )}>
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={cn(
                        ...
                        titleError
                          ? "border-red-500"
                          : "border-border focus:border-orange-500/50",
                      )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        required
                        value={
                          selectedGenres.find((id) =>
                            mainGenres.some((g) => g.slug === id),
                          ) || ""
                        }
                        onChange={(e) => {
                          const genreSlug = e.target.value;
                          if (genreSlug) {
                            const currentMain = selectedGenres.find((id) =>
                              mainGenres.some((g) => g.slug === id),
                            );
                            setSelectedGenres([
                              ...selectedGenres.filter(
                                (id) => id !== genreSlug && id !== currentMain,
                              ),
                              ...(genreSlug ? [genreSlug] : []),
                            ]);
                          }
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        value={
                          selectedGenres.find((id) =>
                            subGenres.some((g) => g.slug === id),
                          ) || ""
                        }
                        onChange={(e) => {
                          const genreSlug = e.target.value;
                          if (genreSlug) {
                            const currentSub = selectedGenres.find((id) =>
                              subGenres.some((g) => g.slug === id),
                            );
                            setSelectedGenres([
                              ...selectedGenres.filter(
                                (id) => id !== genreSlug && id !== currentSub,
                              ),
                              ...(genreSlug ? [genreSlug] : []),
                            ]);
                          }
                    ...
                    <button
                      ...
                      onClick={() => {
                        setFormData((prev) => ({
                          ...prev,
                          visibility:
                            prev.visibility === Visibility.PUBLIC
                              ? Visibility.PRIVATE
                              : Visibility.PUBLIC,
                        }));
                      }}
                    ...
                    <button
                      ...
                      onClick={() => {
                        setFormData((prev) => ({
                          ...prev,
                          status:
                            prev.status === MangaStatus.COMPLETED
                              ? MangaStatus.ONGOING
